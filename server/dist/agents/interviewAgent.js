...
const judgeRunner_1 = require("../tools/judgeRunner");
...
function roleBandFromYears(years) {
    if (years <= 3)
        return "associate_consultant";
    if (years <= 7)
        return "technical_consultant";
    if (years <= 12)
        return "senior_technical_consultant";
    return "technical_architect";
}
function resolveRoleBand(rawRole) {
    if (!rawRole)
        return undefined;
    return ROLE_LOOKUP[normalizeRoleValue(rawRole)];
}
function getRoleDefinition(roleBand) {
...
function resolveRoleContext(profile) {
...
function buildTrackKey(difficulty, roleContext, focusSkill) {
...
function choosePreferredQuestionType(difficulty, yearsExperience) {
    let theoryRatio = difficulty === "easy" ? 0.64 : difficulty === "medium" ? 0.52 : 0.42;
    if (yearsExperience <= 3)
        theoryRatio -= 0.12;
    if (yearsExperience >= 13)
        theoryRatio += 0.12;
...
function signatureForQuestion(title, questionText) {
...
function isRepeatedSignature(signature, recentSignatures) {
...
function rememberSignature(state, trackKey, signature) {
...
function getRecentSignatures(state, trackKey) {
...
function isRepeatedTopic(topic, recentTopics) {
    const normalizedTopic = normalizeTopic(topic);
    if (!normalizedTopic)
        return false;
    if (recentTopics.includes(normalizedTopic))
        return true;
...
function rememberTopic(state, trackKey, topic) {
    const normalizedTopic = normalizeTopic(topic);
    if (!normalizedTopic)
        return;
...
function scoreAnswer(answerText, canonicalAnswer) {
...
function feedbackFromScore(score) {
    if (score >= 85)
        return "Strong answer. You covered core concepts with good depth and structure.";
    if (score >= 65)
        return "Good baseline. Add more explicit trade-offs and edge-case handling.";
    if (score >= 40)
        return "Partial answer. Some ideas are correct, but important details are missing.";
    return "Answer is shallow/incomplete. Cover fundamentals first, then implementation trade-offs.";
}
function isComparableExpected(expected) {
    if (!expected || !expected.trim())
        return false;
    return !expected.includes("...");
}
function parseHarnessResults(stdout) {
...
function buildCodingHarness(source, codingSpec) {
    const marker = "__RESULT__:";
...
function outputMatchesExpected(stdout, expected) {
...
function coerceGeneratedPayload(raw, preferredType) {
    const obj = (raw ?? {});
    const type = obj.type === "coding" || obj.type === "theory" ? obj.type : preferredType;
...
function pickNovelTopic(topicHints, recentTopics) {
...
function detectQuestionTopic(question, topicHints) {
...
function detectSkillTrack(skills) {
    const joined = skills.join(" ").toLowerCase();
    if (joined.includes("drop"))
        return "adobe_dropins";
    if (joined.includes("eds"))
        return "adobe_eds";
    if (joined.includes("system design"))
        return "system_design";
    if (joined.includes("preact"))
        return "preact";
    if (joined.includes("react"))
        return "react";
    if (joined.includes("javascript") || joined.includes("js"))
        return "javascript";
    return "general";
}
const CORE_TOPIC_HINTS_BY_TRACK = {
    javascript: {
        easy: [
...
function isToyCodingQuestion(question, difficulty, roleContext) {
    if (question.type !== "coding")
        return false;
    if (difficulty !== "hard" && roleContext.yearsExperience < 8)
        return false;
    ...
    return (trivialMarkers.some((marker) => text.includes(marker)) ||
...
function buildEmergencyDynamicQuestion(input) {
    ...
    const effectiveType = skillTrack === "system_design" ? "theory" : input.preferredType;
    const includeCommerceFlavor = input.mustIncludeEds ||
        normalizedSkills.some((skill) => skill.includes("adobe") || skill.includes("commerce") || skill.includes("drop"));
    if (effectiveType === "coding") {
        const codingVariantsByTrack = {
            javascript: {
                easy: [
                    {
                        title: "String Utility: Build Slug",
                        questionText: "Implement buildSlug(input) to return a lowercase dash-separated slug by removing punctuation and collapsing separators.",
                        canonicalAnswer: "Lowercase, trim, remove non-word separators, collapse repeated whitespace/dashes, and return deterministic output.",
                        referenceSolution: "function buildSlug(input) {\n  return String(input ?? '')\n    .toLowerCase()\n    .trim()\n    .replace(/[^\\w\\s-]/g, '')\n    .replace(/\\s+/g, '-')\n    .replace(/-+/g, '-');\n}",
                        codingSpec: {
                    ...
                    },
                    {
                        ...
                        questionText: "Implement countFrequencies(items) to return an object mapping normalized string values to counts.",
                        canonicalAnswer: "Normalize each value (trim + lowercase), skip empties, and count deterministically.",
                        referenceSolution: "function countFrequencies(items) {\n  const out = {};\n  for (const item of items) {\n    const key = String(item ?? '').trim().toLowerCase();\n    if (!key) continue;\n    out[key] = (out[key] ?? 0) + 1;\n  }\n  return out;\n}",
                        codingSpec: {
                    ...
                    },
                    {
                        ...
                        questionText: "Implement flattenOneLevel(arr) that flattens one nesting level from an array and preserves element order.",
                        canonicalAnswer: "Iterate through items, spread nested arrays by one level only, and keep non-array values unchanged in order.",
                        referenceSolution: "function flattenOneLevel(arr) {\n  const out = [];\n  for (const item of arr) {\n    if (Array.isArray(item)) out.push(...item);\n    else out.push(item);\n  }\n  return out;\n}",
                        codingSpec: {
                    ...
                    },
                    {
                        ...
                        questionText: "Implement getNestedValue(obj, path, fallback) where path is dot-separated (e.g. 'a.b.c') and return fallback if missing.",
                        canonicalAnswer: "Split path by dots, traverse safely, and return fallback when a segment is missing or nullish.",
                        referenceSolution: "function getNestedValue(obj, path, fallback) {\n  const parts = String(path ?? '').split('.').filter(Boolean);\n  let cur = obj;\n  for (const part of parts) {\n    if (cur == null || !(part in cur)) return fallback;\n    cur = cur[part];\n  }\n  return cur === undefined ? fallback : cur;\n}",
                        codingSpec: {
                ...
                hard: [
                    {
                        ...
                        questionText: "Implement resolveDependencyOrder(modules) where each module has shape {id, deps:string[]}. Return module ids in dependency-safe order, or null when a cycle exists.",
                        canonicalAnswer: "Build adjacency and indegree maps, perform topological sort, and detect cycles by comparing processed count to total modules.",
                        referenceSolution: "function resolveDependencyOrder(modules) {\n  const byId = new Map();\n  const indegree = new Map();\n  const graph = new Map();\n  for (const mod of modules) {\n    const id = String(mod?.id ?? '').trim();\n    if (!id) continue;\n    byId.set(id, mod);\n    if (!graph.has(id)) graph.set(id, []);\n    if (!indegree.has(id)) indegree.set(id, 0);\n  }\n  for (const mod of byId.values()) {\n    const id = String(mod.id);\n    const deps = Array.isArray(mod.deps) ? mod.deps : [];\n    for (const dep of deps) {\n      if (!byId.has(dep)) return null;\n      graph.get(dep).push(id);\n      indegree.set(id, (indegree.get(id) ?? 0) + 1);\n    }\n  }\n  const queue = [...indegree.entries()].filter(([, value]) => value === 0).map(([id]) => id).sort();\n  const out = [];\n  while (queue.length > 0) {\n    const id = queue.shift();\n    out.push(id);\n    for (const next of graph.get(id) ?? []) {\n      indegree.set(next, (indegree.get(next) ?? 0) - 1);\n      if ((indegree.get(next) ?? 0) === 0) {\n        queue.push(next);\n        queue.sort();\n      }\n    }\n  }\n  return out.length === byId.size ? out : null;\n}",
                        codingSpec: {
                    ...
                    },
                    {
                        ...
                        questionText: "Implement diffRecordsById(previous, next) to return {added, removed, updated}. Records share an `id`; `updated` should contain {before, after} for changed records only.",
                        canonicalAnswer: "Index both collections by id, preserve deterministic ordering from the input arrays, and emit added, removed, and shallowly changed records separately.",
                        referenceSolution: "function diffRecordsById(previous, next) {\n  const prevList = Array.isArray(previous) ? previous : [];\n  const nextList = Array.isArray(next) ? next : [];\n  const prevMap = new Map(prevList.map((item) => [item.id, item]));\n  const nextMap = new Map(nextList.map((item) => [item.id, item]));\n  const added = nextList.filter((item) => !prevMap.has(item.id));\n  const removed = prevList.filter((item) => !nextMap.has(item.id));\n  const updated = [];\n  for (const item of nextList) {\n    if (!prevMap.has(item.id)) continue;\n    const before = prevMap.get(item.id);\n    const after = item;\n    if (JSON.stringify(before) !== JSON.stringify(after)) {\n      updated.push({ before, after });\n    }\n  }\n  return { added, removed, updated };\n}",
                        codingSpec: {
                    ...
                    },
                    {
                        ...
                        questionText: "Implement mergeAvailabilityWindows(windows) where each item is {start:number,end:number}. Sort by start and merge overlapping windows into the minimal list.",
                        canonicalAnswer: "Normalize ordering, sort by start, and fold the list by merging each overlapping interval into the last committed interval.",
                        referenceSolution: "function mergeAvailabilityWindows(windows) {\n  const list = [...(Array.isArray(windows) ? windows : [])]\n    .filter((item) => Number.isFinite(item?.start) && Number.isFinite(item?.end))\n    .map((item) => ({ start: Number(item.start), end: Number(item.end) }))\n    .sort((a, b) => a.start - b.start || a.end - b.end);\n  const out = [];\n  for (const window of list) {\n    const last = out[out.length - 1];\n    if (!last || window.start > last.end) {\n      out.push({ ...window });\n      continue;\n    }\n    last.end = Math.max(last.end, window.end);\n  }\n  return out;\n}",
                        codingSpec: {
                ...
                easy: [
                    {
                        ...
                        questionText: "Implement buildProductViewModel(products, query) to filter by case-insensitive query, sort by name, and return {id,name} objects.",
                        canonicalAnswer: "Filter with normalized query, sort in deterministic order, and return minimal shape consumed by UI.",
                        referenceSolution: "function buildProductViewModel(products, query) {\n  const q = String(query ?? '').trim().toLowerCase();\n  return products\n    .filter((p) => {\n      if (!q) return true;\n      return String(p?.name ?? '').toLowerCase().includes(q);\n    })\n    .sort((a, b) => String(a?.name ?? '').localeCompare(String(b?.name ?? '')))\n    .map((p) => ({ id: p.id, name: p.name }));\n}",
                        codingSpec: {
                    ...
                    },
                    {
                        ...
                        questionText: "Implement applyCounterAction(state, action) for a reducer supporting {type:'inc'}, {type:'dec'}, and {type:'reset', value:number}.",
                        canonicalAnswer: "Return a new state object, handle known action types deterministically, and default to unchanged state for unknown actions.",
                        referenceSolution: "function applyCounterAction(state, action) {\n  const current = Number(state?.count ?? 0);\n  if (action?.type === 'inc') return { count: current + 1 };\n  if (action?.type === 'dec') return { count: current - 1 };\n  if (action?.type === 'reset') return { count: Number(action?.value ?? 0) };\n  return { count: current };\n}",
                        codingSpec: {
                    ...
                    },
                    {
                        ...
                        questionText: "Implement reconcileSelection(selectedIds, nextItems) to keep only selected ids that still exist in nextItems.",
                        canonicalAnswer: "Build a set of valid ids from nextItems and filter selectedIds against it while preserving original order.",
                        referenceSolution: "function reconcileSelection(selectedIds, nextItems) {\n  const valid = new Set(nextItems.map((item) => item.id));\n  return selectedIds.filter((id) => valid.has(id));\n}",
                        codingSpec: {
                ...
                hard: [
                    {
                        ...
                        questionText: "Implement buildDirtyPatch(initial, current) to return an object containing only changed keys. Primitive changes should keep the new value; nested plain-object changes should return the full nested object.",
                        canonicalAnswer: "Walk the current object, compare against the initial snapshot, and emit only keys whose serialized value changed, preserving full replacement for changed nested objects.",
                        referenceSolution: "function buildDirtyPatch(initial, current) {\n  const base = initial && typeof initial === 'object' ? initial : {};\n  const next = current && typeof current === 'object' ? current : {};\n  const out = {};\n  for (const key of Object.keys(next)) {\n    const before = base[key];\n    const after = next[key];\n    if (JSON.stringify(before) !== JSON.stringify(after)) {\n      out[key] = after;\n    }\n  }\n  return out;\n}",
                        codingSpec: {
                    ...
                    },
                    {
                        ...
                        questionText: "Implement reconcileNormalizedStore(state, incomingItems) where state is {byId, allIds, selectedId}. Return a new normalized store from incomingItems and keep selectedId only if it still exists.",
                        canonicalAnswer: "Rebuild normalized state from incoming items in deterministic order and drop stale selection that no longer points at a valid entity.",
                        referenceSolution: "function reconcileNormalizedStore(state, incomingItems) {\n  const items = Array.isArray(incomingItems) ? incomingItems : [];\n  const byId = {};\n  const allIds = [];\n  for (const item of items) {\n    const id = item?.id;\n    if (id == null) continue;\n    byId[id] = item;\n    allIds.push(id);\n  }\n  const selectedId = byId[state?.selectedId] ? state.selectedId : null;\n  return { byId, allIds, selectedId };\n}",
                        codingSpec: {
                ...
                easy: [
                    {
                        ...
                        questionText: "Implement deriveVisibleItems(items, selectedCategory) to return items in selected category, sorted by priority descending.",
                        canonicalAnswer: "Filter by selected category, then sort by numeric priority descending for predictable rendering order.",
                        referenceSolution: "function deriveVisibleItems(items, selectedCategory) {\n  return items\n    .filter((item) => String(item?.category ?? '') === String(selectedCategory ?? ''))\n    .sort((a, b) => Number(b?.priority ?? 0) - Number(a?.priority ?? 0));\n}",
                        codingSpec: {
                ...
                easy: [
                    {
                        ...
                        questionText: "Implement normalizeEdsBlocks(blocks) to keep only enabled blocks, trim ids, and return unique blocks by id.",
                        canonicalAnswer: "Filter enabled blocks, normalize ids, and deduplicate by id while preserving deterministic ordering.",
                        referenceSolution: "function normalizeEdsBlocks(blocks) {\n  const seen = new Set();\n  const out = [];\n  for (const block of blocks) {\n    if (!block?.enabled) continue;\n    const id = String(block?.id ?? '').trim();\n    if (!id || seen.has(id)) continue;\n    seen.add(id);\n    out.push({ ...block, id });\n  }\n  return out;\n}",
                        codingSpec: {
                ...
                easy: [
                    {
                        ...
                        questionText: "Implement buildFacetCounts(values) that returns an object with lowercase trimmed keys and occurrence counts.",
                        canonicalAnswer: "Normalize values by trim+lowercase, skip empty strings, then count occurrences in a plain object.",
                        referenceSolution: "function buildFacetCounts(values) {\n  const out = {};\n  for (const raw of values) {\n    const key = String(raw ?? '').trim().toLowerCase();\n    if (!key) continue;\n    out[key] = (out[key] ?? 0) + 1;\n  }\n  return out;\n}",
                        codingSpec: {
                ...
                hard: [
                    {
                        ...
                        questionText: "Implement diffViewModels(previous, next) to return {added, removed, changedIds}. Items have an `id`; `changedIds` must list ids present in both arrays whose serialized content changed.",
                        canonicalAnswer: "Index both arrays by id, preserve deterministic ordering, and separate new items, removed items, and changed shared ids.",
                        referenceSolution: "function diffViewModels(previous, next) {\n  const prevList = Array.isArray(previous) ? previous : [];\n  const nextList = Array.isArray(next) ? next : [];\n  const prevMap = new Map(prevList.map((item) => [item.id, item]));\n  const nextMap = new Map(nextList.map((item) => [item.id, item]));\n  const added = nextList.filter((item) => !prevMap.has(item.id));\n  const removed = prevList.filter((item) => !nextMap.has(item.id));\n  const changedIds = nextList.filter((item) => prevMap.has(item.id) && JSON.stringify(prevMap.get(item.id)) !== JSON.stringify(item)).map((item) => item.id);\n  return { added, removed, changedIds };\n}",
                        codingSpec: {
        ...
        const fallbackGeneralVariants = codingVariantsByTrack.general[input.difficulty] ??
            codingVariantsByTrack.general.medium ??
            codingVariantsByTrack.general.easy ??
            [];
        const codingVariants = variantsForTrack[input.difficulty] ??
...
function questionPayloadToInterviewQuestion(payload, opts) {
...
class InterviewAgent {
    ...
    getSessionState(sessionId, profile) {
    ...
    async nextQuestionHandler(input) {
        ...
        const referenceHits = (0, ragRetriever_1.retrieveReferenceHits)({
            query: [
                difficulty,
        ...
        if (!llmEnabled) {
            ...
            for (const attemptType of typeAttempts) {
                try {
                    ...
                    const candidateTopic = detectQuestionTopic(candidate, allTopicHints) ??
        ...
        if (!selectedQuestion) {
            ...
            if (!selectedQuestion) {
                ...
                if (isToyCodingQuestion(selectedQuestion, difficulty, roleContext)) {
                    ...
                    selectedQuestion = questionPayloadToInterviewQuestion(buildEmergencyDynamicQuestion({
                        difficulty,
    ...
    async evaluateAnswerHandler(input) {
        const sessionId = input.sessionId ?? "default";
        ...
        if (question?.type === "coding") {
            const harnessSource = question.codingSpec
                ...
                : answerText;
            const run = await (0, judgeRunner_1.runCode)({
                language: "javascript",
            ...
            const testResults = harnessResults ??
            ...
            if (questionId)
                state.evaluations.set(questionId, result);
            return result;
        }
